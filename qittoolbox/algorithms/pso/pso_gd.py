...
def pso_gd(cost_f: 'Callable[[np.ndarray],np.ndarray]', gradient_f: 'Callable[[np.ndarray],np.ndarray]', dim: int, x0: np.ndarray = None, v0: np.ndarray=None, **kwargs) -> 'dict':
    """
    Implements the Particle Swarm Optimization (PSO) algorithm using Gradient Descent.

    INPUT:
        cost_f: Callable, will receive an np.ndarray of shape (dim, n_particles), must compute the cost function for each column and return
    ...
    KEYWORD ARGUMENTS:
        ...
        maxiter_gd : integer, maximal number of iterations in the gradient descent algorithm
        ...
        stopping_criterion_gd: Callable(ct_gd, ctp1_gd, iter_idx_gd) -> bool, decision to stop the gradient descent iteration,
    ...
    """
    #TODO: Implement line-search for GD.
    ...
    log(f'[pso_gd] PSO GD running with the following parameters: \n{kwargs}','info')
    

    if not x0 is None and x0.shape != (dim,n_particles):
        log(f'[pso_gd] x0 should have shape (dim,kwargs[n_particles]) = ({dim},{n_particles}), but has x0.shape={x0.shape}. Aborting...','fatal')
        return None
    ...
    if not v0 is None and v0.shape != (dim,n_particles):
         log(f'[pso_gd] v0 should have shape (dim,kwargs[n_particles]) = ({dim},{n_particles}), but has v0.shape={v0.shape}. Aborting...','fatal')
    ...
    log(f'[pso_gd] After init, found best cost ct = {ct} located at index it = {it}, pos = {gt}.','info')
    ...
    log(f'[pso_gd] Initialized, now starting the loop...','info')
    #Do not print the best position if the dimension is too big
    header_str = f'[pso_gd] {"% done" :>8}   {"idx" :>8}   {"weight":>10}   {"ct":>20}   {"it":>8}   {"time":>10}'
    if dim < 6:
         header_str += f'  gt'
    log(header_str, 'info')
    ...
    while not stop_reached:
        ...
        if curr_idx >= log_nextstop:
            progress_str = f'[pso_gd] {round(curr_idx/maxiter*100,2):>8}   {curr_idx:>8}   {curr_weight:>10.3e}   {ct:>20.12e}   {it:>8}   {round(time.time()-_t_since_start,1):>10}'
            if dim < 6:
                progress_str += f'  {gt.T}'
        ...
        if gt_changed_this_iter:
            log(f'[pso_gd] Using gradient descent method as the global optimum has changed since last time, currently ct = {ct}...','debug')
            gt_gd = np.copy(gt)
            ct_gd = ct
            ...
            while not stop_reached_gd:
                ...
                gt_gd -= kwargs['eta_gd'] * grad #TODO: Implement line-search
                ctp1_gd = cost_f(gt_gd)
            ...
            if ct_gd < ct:
                log(f'[pso_gd] Updated the global optimum using gradient descent, from ct={ct} to ct_gd={ct_gd}.','info')
                ct = ct_gd
                gt = gt_gd
