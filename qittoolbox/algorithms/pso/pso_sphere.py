...
def pso_sphere(cost_f: 'Callable[[np.ndarray],np.ndarray]', dim: int, x0: np.ndarray = None, v0: np.ndarray=None, **kwargs) -> 'dict':
    """
    Implements the Particle Swarm Optimization (PSO) algorithm on the unit sphere S^{dim-1} as a subset of R^{dim}.
    ...
    log(f'[pso_sphere] PSO running with the following parameters: \n{kwargs}','info')
    ...
    def sphere_proj(x: np.ndarray, v: np.ndarray) -> np.ndarray:
        ...
        if np.any(np.isnan(outp)):
            log('[pso_sphere] sphere_proj reports isnan values. This is not expected to happen.','error') #something is going wrong
        return outp
    ...
    def sphere_log(x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        if np.any(np.isnan(outp)):
            log('[pso_sphere] sphere_log reports isnan values. This is not expected to happen.','error') #something is going wrong
        return outp
    

    if not x0 is None and x0.shape != (dim,n_particles):
        log(f'[pso_sphere] x0 should have shape (dim,kwargs[n_particles]) = ({dim},{n_particles}), but has x0.shape={x0.shape}. Aborting...','fatal')
        return None
    ...
    if not v0 is None and v0.shape != (dim,n_particles):
         log(f'[pso_sphere] v0 should have shape (dim,kwargs[n_particles]) = ({dim},{n_particles}), but has v0.shape={v0.shape}. Aborting...','fatal')
    ...
    log(f'[pso_sphere] After init, found best cost ct = {ct} located at index it = {it}' + (f', pos = {gt.T}.' if dim < 10 else '') ,'info')
    ...
    log(f'[pso_sphere] Initialized, now starting the loop...','info')
    #Do not print the best position if the dimension is too big
    header_str = f'[pso_sphere] {"% done" :>8}   {"idx" :>8}   {"weight":>10}   {"ct":>20}   {"it":>8}   {"time":>10}'
    if dim < 6:
         header_str += f'  gt'
    log(header_str, 'info')
    ...
    while not stop_reached:
        ...
        if curr_idx >= log_nextstop:
            progress_str = f'[pso_sphere] {round(curr_idx/maxiter*100,2):>8}   {curr_idx:>8}   {curr_weight:>10.3e}   {ct:>20.12e}   {it:>8}   {round(time.time()-_t_since_start,1):>10}'
            if dim < 6:
                progress_str += f'  {gt.T}'
        ...
        if ftp1[itp1] < ct:
            ctp1 = ftp1[itp1]
            gtp1 = xt[:,itp1,None]
            # Fixed by adding None: TODO REMOVE: For broadcasting the shape of gtp1 should always be a column vector!
            # gtp1.shape = (dim,1)
        else:
            ctp1 = ct
